...
{
    public class ContentPageService(ILocalizationService localizationService)
    {
        public string Search(string name, string author, string path, string from)
        {
            ...
            if (Fuzz.Ratio(Path.GetFileNameWithoutExtension(authorPath.Replace(".", "_")),
                author.Replace(".", "_")) < 80)
                return localizationService.Get("SearchDialog.Empty");
            ...
            string fromLoc = localizationService.Get("SearchDialog.From");
